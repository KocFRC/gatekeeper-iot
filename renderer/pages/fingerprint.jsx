...
export default function Fingerprint() {
  ...
  useEffect(function () {
    ...
    window.ipc.send("start_fingerprint_scan", {})
    window.ipc.on("fingerprint_scanned", async function (data) {
      setScannedUser(data)
      if (!data.request) {
        setRequest(null)
      }
      ...
      <div className={`w-full h-full ${scanned ? 'bg-green-600' : 'bg-black'} flex flex-col pb-20`}>
        <div className="flex flex-row mt-3 mx-5 pb-2 border-b-2 border-gray-300">
          <img
            ...
            alt="Logo image"
          />
          <p className="text-2xl font-semibold text-white my-auto ml-4">{scanned ? `${scannedUser.label} scanned successfully.` : 'Please scan your finger.'}</p>
        </div>
        ...
        {request &&
          <div className="rounded-xl mx-5 mt-5 bg-neutral-900 flex flex-row text-white py-4 px-8">
            <div className="flex flex-col">
              ...
              <p className="text-xl">Please use the scanner to record <b>{request.label}</b>.</p>
            </div>
            <TbFingerprintScan className="w-12 h-12 my-auto ml-auto text-rams" />
          </div>
        }
        ...
        <div className="flex flex-row m-auto">
          <IoFingerPrintOutline className="w-32 h-32 text-rams" />
